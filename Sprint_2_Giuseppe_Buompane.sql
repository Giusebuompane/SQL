...
WHERE id not in (SELECT company_id 
				 FROM transaction);
                                                    ...
                                                    -- Opción con not exists ( mas apropriada)
...
select trans.* from transaction as trans 
join company as comp
on trans.company_id= comp.id
where company_name <> 'Non Institute' and comp.country =(SELECT country FROM company
                     where company_name = 'Non Institute');

														-- Opción sin Join 
select * from transaction
where company_id in 
      (select id from company where company_name <> 'Non Institute' and country =
...
select trans.company_id, comp.company_name, count(trans.id) as num_transacciones,
 case 
